...
## [Current Version] beta7preview10

## beta7preview10
1. ポリシー規制サーバーの設定項目に「REQUIRE_MIMIMUM_CLIENT_BUILD」を追加しました。この項目には整数値を指定できます。この項目が指定されている場合は、サーバーに接続しようとしてきたクライアントのビルド番号が、指定された番号未満の場合に、クライアント側に対してバージョンアップを促すエラーメッセージが表示され、接続が拒否されます。「REQUIRE_MIMIMUM_CLIENT_BUILD」の値は、サーバー側のソフトウェアのビルド番号以下でなければなりません。(サーバー側のソフトウェアのビルド番号を超える値が設定されている場合は、サーバー側のソフトウェアのビルド番号が設定されているとみなされます。) この機能は、シン・テレワークシステムのパブリック版では利用できません。
